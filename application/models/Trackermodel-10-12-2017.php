...
class Trackermodel extends CI_Model {
    ...
    function addTracker($region = 0, $market = 0, $vendor = 0, $po_number = '', $line = '', $po_date = '', $site_name = '', $description = '', $unit_price = 0, $amount = 0, $qty = '', $rev = 0, $supplier_no='',$orderArray,$gloablArray) {
		...
		if(is_null($orderArray['delivery_date']))
		{
			$orderArray['delivery_date'] = '';
		}
        //echo $po_date;
        $po_date = date('Y-m-d',strtotime($po_date)); 
        $sql = 'INSERT INTO ' . $this->db->dbprefix($this->trackerTable) . ' (region, market, vendor, po_number, line, po_line_rev, po_date, site_name,description, unit_price, amount, qty, rev, status, supplier_no,uco,delivery_date,fa_code,project_type,project_no,terms,part_no,created_by,order_date)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)
                        ...
                        rev=?,
                        status=?';
        $this->db->query($sql, array($region, $market, $vendor, $po_number, $line, $po_number . $line, $po_date, $site_name, $description, $unit_price, $amount, $qty, $rev, $status, $supplier_no,$orderArray['uco'],$orderArray['delivery_date'],$gloablArray['fa_code'],$gloablArray['project_type'],$gloablArray['project_no'],$gloablArray['terms'],$gloablArray['part_no'],$gloablArray['created_by'],$gloablArray['order_date'],$po_date, $site_name, $description, $unit_price, $amount, $qty, $rev, $status));

                                        ...
                                        foreach ($lines as $linekey => $line) {
											...
											if (preg_match('/Revision Date/', $line)) {
												...
												foreach ($temp as $value) {
                                                    ...
                                                    if (preg_match('/Revision Date/', trim($value)) && !$found) {
														if(count($value))
														{
															$dateTime = trim(str_replace('Revision Date', '', $value));
															if(count($dateTime))
															{
																$gloablArray['revision_date'] = date("Y-m-d", strtotime($dateTime));
															}
															else{
																$gloablArray['revision_date'] = '';
															}
														}
														$gloablArray['revision_date'] = '';
                                                    }
											...
											if (preg_match('/Revised by/', $line)) {
												...
												if(count($temp) > 1)
												{
												    foreach ($temp as $value) {
														if ($found) {
														   $found = FALSE;
															$gloablArray['revised_by'] = trim($value);
															break;
														}
														if (strtolower(trim($value)) == 'revised by') {
												...
												if (preg_match('/Revised by/', trim($line)) && !$found) {
												   $line = trim(str_replace('@@', '', $line));
												   $line = trim(str_replace('Revised by', '', $line));
												  
												   if(count($line))
												   {
													   $gloablArray['revised_by'] = $line;
												   }
												   else{
														$gloablArray['revised_by'] = '';
												   }
                                                   
												   #echo "223334:".$gloablArray['revised_by'];
                                                }
												#exit;
                                            }
